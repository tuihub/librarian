...
func (fq *FileQuery) QueryImage() *ImageQuery {
...
func (fq *FileQuery) FirstID(ctx context.Context) (id model.InternalID, err error) {
...
func (fq *FileQuery) FirstIDX(ctx context.Context) model.InternalID {
...
func (fq *FileQuery) OnlyID(ctx context.Context) (id model.InternalID, err error) {
	var ids []model.InternalID
...
func (fq *FileQuery) OnlyIDX(ctx context.Context) model.InternalID {
...
func (fq *FileQuery) IDs(ctx context.Context) (ids []model.InternalID, err error) {
...
func (fq *FileQuery) IDsX(ctx context.Context) []model.InternalID {
	ids, err := fq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (fq *FileQuery) WithImage(opts ...func(*ImageQuery)) *FileQuery {
	query := (&ImageClient{config: fq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (fq *FileQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*File, error) {
	...
	if query := fq.withImage; query != nil {
		if err := fq.loadImage(ctx, query, nodes, nil,
			func(n *File, e *Image) { n.Edges.Image = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (fq *FileQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*File, init func(*File), assign func(*File, *User)) error {
	...
	nodeids := make(map[model.InternalID][]*File)
	for i := range nodes {
		if nodes[i].user_file == nil {
			continue
		}
		fk := *nodes[i].user_file
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_file" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (fq *FileQuery) loadImage(ctx context.Context, query *ImageQuery, nodes []*File, init func(*File), assign func(*File, *Image)) error {
	...
	for _, n := range neighbors {
		fk := n.file_image
		if fk == nil {
			return fmt.Errorf(`foreign-key "file_image" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
