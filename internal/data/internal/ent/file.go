	...
	for i := range columns {
		...
		case file.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				_m.user_file = new(model.InternalID)
				*_m.user_file = model.InternalID(value.Int64)
			}
		default:
...
func (_m *File) QueryImage() *ImageQuery {
	return NewFileClient(_m.config).QueryImage(_m)
}
