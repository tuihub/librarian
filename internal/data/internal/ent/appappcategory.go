...
func (_m *AppAppCategory) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case appappcategory.FieldAppCategoryID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				_m.AppCategoryID = model.InternalID(value.Int64)
			}
...
func (_m *AppAppCategory) Value(name string) (ent.Value, error) {
	return _m.selectValues.Get(name)
}

// QueryAppCategory queries the "app_category" edge of the AppAppCategory entity.
func (_m *AppAppCategory) QueryAppCategory() *AppCategoryQuery {
	return NewAppAppCategoryClient(_m.config).QueryAppCategory(_m)
}

// QueryApp queries the "app" edge of the AppAppCategory entity.
func (_m *AppAppCategory) QueryApp() *AppQuery {
	return NewAppAppCategoryClient(_m.config).QueryApp(_m)
}
...
func (_m *AppAppCategory) Update() *AppAppCategoryUpdateOne {
	return NewAppAppCategoryClient(_m.config).UpdateOne(_m)
}

// Unwrap unwraps the AppAppCategory entity that was returned from a transaction after it was closed,
// so that all future queries will be executed through the driver which created the transaction.
func (_m *AppAppCategory) Unwrap() *AppAppCategory {
...
func (_m *AppAppCategory) String() string {
	var builder strings.Builder
	builder.WriteString("AppAppCategory(")
	builder.WriteString(fmt.Sprintf("id=%v, ", _m.ID))
	builder.WriteString("app_category_id=")
	builder.WriteString(fmt.Sprintf("%v", _m.AppCategoryID))
	builder.WriteString(", ")
